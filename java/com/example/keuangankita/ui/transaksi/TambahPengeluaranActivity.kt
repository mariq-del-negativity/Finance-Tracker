...
class TambahPengeluaranActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSimpan.setOnClickListener { cekDanSimpan() }
    }

    private fun cekDanSimpan() {
        ...
        val saldoSaatIni = transaksiRepository.getTotalSaldo(userId)

        if (jumlah > saldoSaatIni) {
            tampilkanAlertSaldoKurang(jumlah, saldoSaatIni, keterangan, kategori)
        } else {
            simpanPengeluaran(jumlah, keterangan, kategori)
        }
    }

    private fun tampilkanAlertSaldoKurang(
        jumlah: Double, saldo: Double,
        keterangan: String, kategori: String
    ) {
        AlertDialog.Builder(this)
            .setTitle("⚠️ Saldo Tidak Mencukupi")
            .setMessage(
                "Jumlah pengeluaran ${CurrencyFormatter.format(jumlah)} " +
                "melebihi saldo kamu ${CurrencyFormatter.format(saldo)}.\n\n" +
                "Tetap lanjutkan?"
            )
    ...
    private fun simpanPengeluaran(jumlah: Double, keterangan: String, kategori: String) {
